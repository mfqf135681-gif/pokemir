...
def main():
    ...
    ap.add_argument("--grid", type=int, default=16, help="hash 网格(8粗/16细)")
    ap.add_argument("--first-char", action=argparse.BooleanOptionalAction, default=True, help="只hash第一个字(跟/加/下/过互异)")
    ...
    for tok in args.seed_anchors.split(","):
        ...
        h = text_shape_hash(crop_action(cv2.imread(fp), int(seat_s)), args.sat_th, args.val_th, args.grid, args.first_char)
        if not h:
    ...
    for fp in sampled:
        ...
        for sidx in seat_roi:
            ...
            if c is None:
                continue
            h = text_shape_hash(c, args.sat_th, args.val_th, args.grid, args.first_char)
            if not h:
                continue
    ...
    payload = {"version": 1, "profile": args.profile, "auto_collected": True,
               "sat_th": args.sat_th, "val_th": args.val_th, "grid": args.grid, "first_char": args.first_char,
