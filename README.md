...
| §15 **recall**(真目标 95%) | ⭐ **2 个 CHECK 手干净 --truth 均 recall 100%**(hand4 8/8、hand7 10/10,precision 86% 且 FP 是读偏非幻影)→ **守恒 CHECK=读噪声(补码/误读/None)非漏抓,61% 守恒严重低估捕获**。**瓶颈从来不是 recall**,是切手(已修)+读噪声(治法=数字配方)。⚠️ n=2 单 session 都 heads-up,非全局定论;多人池/all-in/跨录像待全标验 |
