...
> **状态:BLUEPRINT,不实施**。门槛核对(2026-06-12):耐久验收 ✅ / 识别层冻结 ✅(红线 R-5)/ **#227 接口契约 ❌** / Win ✅ → **2/4 未齐**。
> **📅 排程(2026-06-12 定)**:#226 求解器 → #227 接口定型(**让求解器先当 TickObservation 的真实消费者,接口按实际需求定型**)→ 走 R-5 解冻流程开"重构窗口"(§4.3 八刀 + §7 HOLD 项一并)→ 全量审计回归 → 回冻。**新论据**:审计体系+求解器 = §5.3"行为保持不可验"的解药——每刀重构后跑一场,closure 分层+六仪表盘就是现成回归 oracle(蓝图立时不存在的安全网)。
