...
> **2026-06-08 立**。基于全量深读 50 个方法 + 与用户的设计共识。
> **作用**:把现有 orchestrator(Path A 活系统)从 50 方法上帝类**渐进重构**成内聚模块(原地,不换持久化、不上 event-sourcing)。
> **状态:BLUEPRINT,不实施**。门槛 = 识别层冻结 + 契约 #227(见 §5/§6)。
> **性能提升 ≈ 0**(已与用户确认,不以提速为由;目的=正确性/可维护/可测/4模式切换)。
> **关联**:`[[主题-DDD架构]]`(见 §0 关系澄清)· #238/#226/#227 · `[[constraint-solver-paradigm]]` · `[[pillar-confidence-doctrine]]`
