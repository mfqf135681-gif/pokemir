...
> 最近更新:2026-06-15 **续治到 14Hz §0″**(canonicalize O(N²)每手末→增量 / 动作批冗余EasyOCR→phash闸省169ms / timer退役省88ms;median 14Hz)
> 涉及讨论:2026-06-05 讨论+侦察 → 2026-06-06 DB瓶颈落地 → 2026-06-09 提速落地(5.6Hz) → 2026-06-15 续治(14Hz)(本文即正文,无 archive)
> 当前状态:**提速已落地,median ~14Hz**(p10 12.3 / min 9.9,无停顿;一小时局 2961 tick)。瓶颈链 DB✅→截屏✅→canonicalize/动作批/timer✅。**剩余为非-perf 课题**:~17% 丢手=切手/落库缺口(非 perf、非读数,侧验已否)→ #242 升级"双闸门+窗口捕获";盲注注入@高帧抢跑待修(挂 #242)。
...
| 时间 | 子主题 | 状态 | 关键决策 | 原文 |
|:---|:---|:---|:---|:---|
| **2026-06-15** | **续治 5.6→14Hz:三刀 + 侧验否两丢手假设** | **已落地** | canonicalize O(N²)每手末→增量(64s→0.5ms)；动作批冗余EasyOCR→phash闸(省169ms)；timer退役(省88ms)+吞街改换街清；median 14Hz/min 9.9。侧验:高帧↑丢手↓、白占比100%→**17%丢手=切手/落库缺口非perf**→#242升级双闸门 | §0″ |
...
### §0″ 续治到 14Hz(2026-06-15)⭐ 三刀 + 侧验:丢手非 perf

承 §0′(5.6Hz)。注册表/批EasyOCR/timer 三者叠加把帧率又压崩到 ~2Hz(+ 手末 4 秒尖峰),本轮三刀续治到 **median 14Hz**:

- **canonicalize O(N²) 每手末(尖峰元凶)**:`_canonicalize_player_id_map` 对整个玩家注册表(涨到 2654 名)两两模糊匹配 → benchmark 单次 **64s**、手末 4 秒尖峰(随数据涨而崩,10Hz 时注册表小才没暴露)→ 改**增量去重**(只比【本轮新名 × 全集】= O(新×N);启动用载入的注册表**播种"已扫集"**免首手退化 O(N²))→ **0.5ms**。
- **动作批冗余 EasyOCR(稳态大头 169ms)**:`_pre_batch_action_amount_ocr` 在 OCR_BATCH 开时对 8 动作区跑 EasyOCR,但 **phash(#240)开着 → `_process_seat_actions` 先用 phash 设 action_text、`_batched_action_results` 永不被消费 → 纯浪费**。补 phash 闸(与 amount 批的 `not DIGIT_RECIPE_LIVE` 对称)→ 省 169ms。(`sa[cap/diff/phash/pre/ocrfb]` 子计时探针锁定:phash 本身仅 0.4ms,169ms 全在 pre_batch;探针留用。)
- **timer 整套退役(88ms,双OCR遗物)**:决策耗时/timebank = 全仓**只写不读死数据** → 删(要时用相邻动作 `created_at` 时间戳反推,属解释层、不占识别层 OCR);**"过牌→过牌"跨街被 `check_action_change` 同字吞的吞街保护,从 timer 代理移到【换街清 `_prev_action_texts`】**(`_process_community_cards` 新公共牌检出处——直接确定信号,比 timer"恰好读到倒计时才清"覆盖全)。
- **结果**:一小时局 2961 tick → **median 14Hz / p10 12.3 / min 9.9(无多秒停顿)**。

**侧验(perf hunt 顺带数据驱动否两个丢手假设,见 [[feedback-data-driven-mandate]])**:
- ① "**高帧致丢手**"被否:跨天 帧率↑ 丢手↓(1.1Hz=16-26%、7-8Hz=6-12%、14Hz=12%);低帧采样稀反而更易错过按钮转场。**别砍帧率**(砍了丢手更多 + 把漏抓放回来)。
- ② "**读数致丢手**"被否:本局 button.detected 白占比 **89/89=100%**(非老注释 51/84 崩)。
- → **~17% 丢手 = 切手逻辑/落库缺口**(检测 button89/pot87/init87 全 < 真实~99、录 81),**非 perf 非读数**。

**衍生(非 perf,挂 [[#242]])**:盲注注入@高帧"活跃集第一次非空就派"抢在发牌动画前 → ante 漏派/SB-BB 错。澄清:**总底池=手结束闸、按钮=手开始闸,中间有时间窗** = 端点(承重锚)/赢家/摊牌的可靠捕获窗 → #242 升级为"**双闸门(两闸建牢)+ 窗口捕获 + 落库缺口修**"地基课题(原则:漏=信息丢不可恢复、多=求解器端点 trim 可恢复 → 强制时选可恢复的错)。
